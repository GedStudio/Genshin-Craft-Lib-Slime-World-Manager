...
public class WorldImporter {
    ...
    public static SlimeLoadedWorld readFromDirectory(File worldDir) throws InvalidWorldException, IOException {
        ...
        if (worldVersion != SlimeManagerImpl.getInstance().getNms().getWorldVersion()) {
            throw new UnsupportedOperationException("Please ensure that this world is not outdated.");
        }

        return SlimeManagerImpl.getInstance().getNms().createSlimeWorld(null, worldDir.getName(), chunks, new CompoundTag("", extraData), maps, worldVersion, propertyMap, false, true, null);
    }
